...
describe("nojs init generator", () => {
	it("generates files in the given directory (not a subdirectory)", async () => {
		...
		const result = await generate(
			{ name: "test-app", routing: false, i18n: false, apiUrl: null },
			cwd,
		);
	...
	it("generates a project with routing", async () => {
		const cwd = await makeTempDir();
		const result = await generate(
			{ name: "spa-app", routing: true, i18n: false, apiUrl: null },
			cwd,
		);
	...
	it("generates a project with i18n", async () => {
		...
		const enJson = JSON.parse(
			await readFile(join(cwd, "locales", "en.json"), "utf-8"),
		);
		expect(enJson.greeting).toBe("Hello, No.JS!");

		const ptJson = JSON.parse(
			await readFile(join(cwd, "locales", "pt.json"), "utf-8"),
		);
	...
	it("generates nojs.config.json with project metadata", async () => {
		const cwd = await makeTempDir();
		await generate(
			{ name: "config-test", routing: false, i18n: false, apiUrl: null },
			cwd,
		);

		const config = JSON.parse(
			await readFile(join(cwd, "nojs.config.json"), "utf-8"),
		);
