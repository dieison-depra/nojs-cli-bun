...
async function writeTestHtml(name, content) {
	...
	await mkdir(join(testDir, ...name.split("/").slice(0, -1)), {
		recursive: true,
	});
...
describe("prebuild runner", () => {
	...
	it("throws on unknown plugin name", async () => {
		...
		await expect(
			prebuild({ cwd: testDir, plugins: { nonexistent: true } }),
...
describe("inject-resource-hints", () => {
	...
	it("skips interpolated URLs", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><div get="/api/users/{id}" as="user"></div></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inject-resource-hints": true },
	...
	it("injects preconnect for cross-origin URLs", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><div get="https://api.example.com/data" as="data"></div></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inject-resource-hints": true },
...
describe("inject-resource-hints — B3/B4 extensions", () => {
	...
	it("B3: does not duplicate locale preload if already present", async () => {
		await writeTestHtml(
			"index.html",
			'<html lang="pt-BR"><head><link rel="preload" as="fetch" href="/locales/pt-BR.json" crossorigin="anonymous"></head><body></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inject-resource-hints": true },
	...
	it("B4: does not duplicate dns-prefetch if already present", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><link rel="dns-prefetch" href="https://api.example.com"></head><body></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: {
...
describe("inject-head-attrs", () => {
	it("injects title from static page-title", async () => {
		await writeTestHtml(
			"index.html",
			"<html><head></head><body page-title=\"'My Site'\"></body></html>",
		);
	...
	it("skips dynamic expressions", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body page-title="pageTitle"></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inject-head-attrs": true } });
		const html = await readTestHtml("index.html");
...
describe("inject-speculation-rules", () => {
	it("injects speculation rules from route definitions", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><template route="/"></template><template route="/about"></template></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inject-speculation-rules": true },
		});
		const html = await readTestHtml("index.html");
	...
	it("skips parameterized routes", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><template route="/users/:id"></template><template route="/about"></template></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inject-speculation-rules": true },
		...
		const rules = JSON.parse(
			html.match(/<script type="speculationrules">([\s\S]*?)<\/script>/)?.[1] ||
				"{}",
		);
...
describe("optimize-images", () => {
	it("adds lazy loading to non-LCP images", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><img src="/hero.jpg"><img src="/thumb.jpg"></body></html>',
		);
...
describe("inline-animation-css", () => {
	it("injects only the keyframes used in the document", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><div animate="fadeIn"></div><div animate-leave="fadeOut"></div></body></html>',
		);
	...
	it("does not inject if no animation directives are present", async () => {
		await writeTestHtml(
			"index.html",
			"<html><head></head><body><p>No animations</p></body></html>",
		);
		await prebuild({ cwd: testDir, plugins: { "inline-animation-css": true } });
		const html = await readTestHtml("index.html");
	...
	it("skips injection if data-nojs-animations already present", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><style data-nojs-animations=""></style></head><body><div animate="fadeIn"></div></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inline-animation-css": true } });
		const html = await readTestHtml("index.html");
	...
	it("ignores unknown animation names", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><div animate="customSlide"></div></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inline-animation-css": true } });
		const html = await readTestHtml("index.html");
		expect(html).not.toContain("data-nojs-animations");
	...
	it("handles animate-enter attribute", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><div animate-enter="slideInLeft"></div></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inline-animation-css": true } });
		const html = await readTestHtml("index.html");
...
describe("inject-visibility-css", () => {
	it("marks if= elements with data-nojs-pending and injects CSS", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><div if="showModal">Modal</div></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inject-visibility-css": true },
		});
		const html = await readTestHtml("index.html");
	...
	it("marks show= and hide= elements", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><div show="visible">A</div><div hide="hidden">B</div></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inject-visibility-css": true },
	...
	it("does not mark elements inside <template>", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><template><div if="x">inside template</div></template><div if="y">outside</div></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inject-visibility-css": true },
	...
	it("does not inject if no conditional elements present", async () => {
		await writeTestHtml(
			"index.html",
			"<html><head></head><body><p>No conditionals</p></body></html>",
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inject-visibility-css": true },
	...
	it("injects the CSS block only once with multiple elements", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><div if="a">A</div><div show="b">B</div></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inject-visibility-css": true },
...
describe("inject-template-hints", () => {
	it("preloads stylesheets when a loading= template is referenced", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "inject-template-hints": true },
		});
		const html = await readTestHtml("index.html");
		expect(html).toContain('rel="preload"');
	...
	it("does nothing when no slot attributes are present", async () => {
		await writeTestHtml(
			"index.html",
			"<html><head></head><body><p>No templates</p></body></html>",
		);
...
describe("minify-html", () => {
	...
	it("removes HTML comments", async () => {
		await writeTestHtml(
			"index.html",
			"<html><head></head><body><!-- this is a comment --><p>Hello</p></body></html>",
		);
		await prebuild({ cwd: testDir, plugins: { "minify-html": true } });
		const html = await readTestHtml("index.html");
	...
	it("preserves content inside <script> tags", async () => {
		...
		await writeTestHtml(
			"index.html",
			`<html><head>${scriptContent}</head><body></body></html>`,
		);
		await prebuild({ cwd: testDir, plugins: { "minify-html": true } });
		const html = await readTestHtml("index.html");
	...
	it("preserves content inside <style> tags", async () => {
		...
		await writeTestHtml(
			"index.html",
			`<html><head>${styleContent}</head><body></body></html>`,
		);
		await prebuild({ cwd: testDir, plugins: { "minify-html": true } });
		const html = await readTestHtml("index.html");
	...
	it("preserves IE conditional comments", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><!--[if IE]><link rel="stylesheet" href="ie.css"><![endif]--></head><body></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "minify-html": true } });
		const html = await readTestHtml("index.html");
	...
	it("respects removeComments: false config option", async () => {
		await writeTestHtml(
			"index.html",
			"<html><head></head><body><!-- keep me --><p>Hello</p></body></html>",
		);
		await prebuild({
			cwd: testDir,
			plugins: { "minify-html": { removeComments: false } },
		});
		const html = await readTestHtml("index.html");
...
describe("inject-view-transitions", () => {
	it("injects @view-transition CSS into head", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "inject-view-transitions": true },
		});
		const html = await readTestHtml("index.html");
	...
	it("does nothing when config.enabled === false", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "inject-view-transitions": { enabled: false } },
		});
		const html = await readTestHtml("index.html");
...
describe("inject-modulepreload", () => {
	...
	it("skips inline module scripts without src", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><script type="module">console.log("hi");</script></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inject-modulepreload": true } });
		const html = await readTestHtml("index.html");
		expect(html).not.toContain('rel="modulepreload"');
	...
	it("skips cross-origin module scripts", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><script type="module" src="https://cdn.example.com/app.js"></script></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inject-modulepreload": true } });
		const html = await readTestHtml("index.html");
		expect(html).not.toContain('rel="modulepreload"');
	...
	it("skips interpolated src values", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><script type="module" src="/app.{hash}.js"></script></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inject-modulepreload": true } });
		const html = await readTestHtml("index.html");
		expect(html).not.toContain('rel="modulepreload"');
	...
	it("does not duplicate modulepreload if already present", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><link rel="modulepreload" href="/app.js"></head><body><script type="module" src="/app.js"></script></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inject-modulepreload": true } });
		const html = await readTestHtml("index.html");
...
describe("audit-meta-tags", () => {
	...
	it("throws when failOnError: true and title is missing", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="description" content="test"></head><body></body></html>',
		);
		await expect(
			prebuild({
				cwd: testDir,
				plugins: { "audit-meta-tags": { failOnError: true } },
			}),
	...
	it("does not modify HTML when all tags already present", async () => {
		const original =
			'<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Test</title><meta name="description" content="Test page"></head><body></body></html>';
...
describe("inject-canonical-url", () => {
	...
	it("does not inject when siteUrl is not set", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "inject-canonical-url": { cwd: testDir } },
		});
		const html = await readTestHtml("index.html");
	...
	it("does not duplicate if canonical already present", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><link rel="canonical" href="https://other.com/"></head><body></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: {
...
describe("enforce-script-loading", () => {
	it("adds defer to third-party scripts without defer/async", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "enforce-script-loading": true },
		});
		const html = await readTestHtml("index.html");
	...
	it("does not modify scripts that already have defer", async () => {
		const original =
			'<html><head><script src="https://cdn.example.com/lib.js" defer></script></head><body></body></html>';
		await writeTestHtml("index.html", original);
		await prebuild({
			cwd: testDir,
			plugins: { "enforce-script-loading": true },
	...
	it("does not modify scripts that already have async", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><script src="https://cdn.example.com/lib.js" async></script></head><body></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "enforce-script-loading": true },
	...
	it("does not modify same-origin scripts", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><script src="/local/lib.js"></script></head><body></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "enforce-script-loading": true },
	...
	it('does not modify <script type="module"> tags', async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><script src="https://cdn.example.com/mod.js" type="module"></script></head><body></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "enforce-script-loading": true },
	...
	it("uses async when config.strategy is async", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "enforce-script-loading": { strategy: "async" } },
		});
		const html = await readTestHtml("index.html");
		expect(html).toContain("async");
		expect(html).not.toContain("defer");
	...
	it("skips scripts from hosts in the allowList", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "enforce-script-loading": { allowList: ["cdn.example.com"] } },
		});
		const html = await readTestHtml("index.html");
		expect(html).not.toContain("defer");
		expect(html).not.toContain("async");
...
describe("inject-csp-hashes", () => {
	...
	it("handles both inline script and style in same document", async () => {
		await writeTestHtml(
			"index.html",
			"<html><head><style>body { margin: 0; }</style></head><body><script>var x = 1;</script></body></html>",
		);
		await prebuild({ cwd: testDir, plugins: { "inject-csp-hashes": true } });
		const html = await readTestHtml("index.html");
		expect(html).toContain("Content-Security-Policy");
	...
	it("does not hash script type speculationrules", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><script type="speculationrules">{"prerender":[]}</script></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inject-csp-hashes": true } });
		const html = await readTestHtml("index.html");
		expect(html).not.toContain("Content-Security-Policy");
	...
	it("updates existing CSP meta tag without duplicating", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><meta http-equiv="Content-Security-Policy" content="default-src \'self\'"></head><body><script>alert(1);</script></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inject-csp-hashes": true } });
		const html = await readTestHtml("index.html");
...
describe("inject-sri-hashes", () => {
	...
	it("does not break the build when the URL is unreachable", async () => {
		...
		await expect(
			prebuild({
				cwd: testDir,
				plugins: { "inject-sri-hashes": { timeout: 500 } },
			}),
		).resolves.toBeDefined();
		const html = await readTestHtml("index.html");
		expect(html).not.toContain("integrity=");
...
describe("precompress-assets", () => {
	it("generates .br companion for HTML files when brotli: true", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "precompress-assets": { brotli: true, gzip: false } },
		});
	...
	it("generates .gz companion for HTML files when gzip: true", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "precompress-assets": { brotli: false, gzip: true } },
		});
	...
	it("also compresses .css and .js files if present in the output dir", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "precompress-assets": { brotli: true, gzip: true } },
		});
...
describe("inject-jsonld", () => {
	...
	it("includes description from meta description", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><title>My Page</title><meta name="description" content="A great page"></head><body></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inject-jsonld": true } });
		const html = await readTestHtml("index.html");
		const match = html.match(
	...
	it("includes url from canonical link", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><title>My Page</title><link rel="canonical" href="https://example.com/my-page"></head><body></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inject-jsonld": true } });
		const html = await readTestHtml("index.html");
		const match = html.match(
	...
	it("includes inLanguage from <html lang>", async () => {
		await writeTestHtml(
			"index.html",
			'<html lang="pt-BR"><head><title>Minha Página</title></head><body></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "inject-jsonld": true } });
		const html = await readTestHtml("index.html");
		const match = html.match(
	...
	it("does not inject when LD+JSON already exists (idempotent)", async () => {
		...
		await writeTestHtml(
			"index.html",
			`<html><head><title>My Page</title><script type="application/ld+json">${existing}</script></head><body></body></html>`,
		);
		await prebuild({ cwd: testDir, plugins: { "inject-jsonld": true } });
		const html = await readTestHtml("index.html");
	...
	it("respects config.type === WebSite", async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "inject-jsonld": { type: "WebSite" } },
		});
		const html = await readTestHtml("index.html");
		const match = html.match(
			/<script type="application\/ld\+json">([\s\S]*?)<\/script>/,
...
describe("inline-critical-css", () => {
	it("returns HTML unchanged when beasties is not installed", async () => {
		const input =
			'<html><head><link rel="stylesheet" href="/styles.css"></head><body><h1>Hello</h1></body></html>';
		await writeTestHtml("index.html", input);
	...
	it("plugin can be enabled without crashing", async () => {
		...
		const result = await prebuild({
			cwd: testDir,
			plugins: { "inline-critical-css": true },
		});
...
describe("optimize-fonts", () => {
	it("injects preconnect for fonts.googleapis.com when Google Fonts link is present", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Roboto"></head><body></body></html>',
		);
	...
	it("does not duplicate preconnects if already present", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><link rel="preconnect" href="https://fonts.googleapis.com" crossorigin><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Roboto&display=swap"></head><body></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "optimize-fonts": true } });
		const html = await readTestHtml("index.html");
		const apiMatches = (
			html.match(/href="https:\/\/fonts\.googleapis\.com"/g) || []
		).length;
		const staticMatches = (
			html.match(/href="https:\/\/fonts\.gstatic\.com"/g) || []
		).length;
	...
	it("does not duplicate display=swap if already present", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Roboto&display=swap"></head><body></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "optimize-fonts": true } });
		const html = await readTestHtml("index.html");
...
describe("generate-responsive-images", () => {
	...
	it("does not throw when sharp is absent", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><img src="/photo.png"></body></html>',
		);
		await expect(
			prebuild({
				cwd: testDir,
...
describe("generate-pwa-manifest", () => {
	it('injects <link rel="manifest"> into head', async () => {
		...
		await prebuild({
			cwd: testDir,
			plugins: { "generate-pwa-manifest": true },
		});
		const html = await readTestHtml("index.html");
...
describe("purge-unused-css", () => {
	it("removes CSS class rules with no matching elements in HTML", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><style>.unused { color: red; } .used { color: blue; }</style></head><body><div class="used"></div></body></html>',
		);
	...
	it("keeps CSS rules that have matching elements", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><style>.card { padding: 1rem; } p { margin: 0; }</style></head><body><div class="card"><p>Hello</p></div></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "purge-unused-css": true } });
		const html = await readTestHtml("index.html");
	...
	it("keeps @keyframes at-rules without purging", async () => {
		await writeTestHtml(
			"index.html",
			"<html><head><style>@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } } .unused { color: red; }</style></head><body></body></html>",
		);
		await prebuild({ cwd: testDir, plugins: { "purge-unused-css": true } });
		const html = await readTestHtml("index.html");
	...
	it("keeps @media rules without purging", async () => {
		await writeTestHtml(
			"index.html",
			"<html><head><style>@media (max-width: 768px) { .sidebar { display: none; } } .gone { color: green; }</style></head><body></body></html>",
		);
		await prebuild({ cwd: testDir, plugins: { "purge-unused-css": true } });
		const html = await readTestHtml("index.html");
	...
	it("does not touch NoJS internal style tags", async () => {
		...
		await writeTestHtml(
			"index.html",
			`<html><head><style data-nojs-animations>${css}</style></head><body></body></html>`,
		);
		await prebuild({ cwd: testDir, plugins: { "purge-unused-css": true } });
		const html = await readTestHtml("index.html");
	...
	it("handles multiple selectors: keeps rule if ANY selector matches", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head><style>.missing, .present { font-size: 1rem; }</style></head><body><div class="present"></div></body></html>',
		);
		await prebuild({ cwd: testDir, plugins: { "purge-unused-css": true } });
		const html = await readTestHtml("index.html");
	...
	it("is a no-op when all CSS rules are used", async () => {
		const original =
			"<html><head><style>h1 { color: black; } p { margin: 0; }</style></head><body><h1>Title</h1><p>Text</p></body></html>";
		await writeTestHtml("index.html", original);
		await prebuild({ cwd: testDir, plugins: { "purge-unused-css": true } });
		const html = await readTestHtml("index.html");
...
describe("audit-accessibility", () => {
	it("does not throw for compliant HTML", async () => {
		...
		await expect(
			prebuild({ cwd: testDir, plugins: { "audit-accessibility": true } }),
		).resolves.toBeDefined();
	});
	...
	it("throws when failOnError is true and <img> has no alt", async () => {
		await writeTestHtml(
			"index.html",
			'<html lang="en"><head></head><body><img src="/hero.jpg"></body></html>',
		);
		await expect(
			prebuild({
				cwd: testDir,
	...
	it("does not throw when failOnError is false despite violations", async () => {
		...
		await expect(
			prebuild({
				cwd: testDir,
				plugins: { "audit-accessibility": { failOnError: false } },
			}),
		).resolves.toBeDefined();
	});
	...
	it("returns HTML unchanged (audit only, no DOM mutation)", async () => {
		const original =
			'<html lang="en"><head></head><body><img src="/hero.jpg" alt="Hero"></body></html>';
		await writeTestHtml("index.html", original);
...
describe("inline-svg", () => {
	it("inlines a small SVG file", async () => {
		await writeFile(
			join(testDir, "icon.svg"),
			'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"><circle cx="12" cy="12" r="10"/></svg>',
		);
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><img src="/icon.svg" alt="icon"></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inline-svg": { cwd: testDir } },
		});
		const html = await readTestHtml("index.html");
	...
	it("does not inline SVG files larger than maxBytes", async () => {
		const bigSvg =
			'<svg xmlns="http://www.w3.org/2000/svg">' + "x".repeat(5000) + "</svg>";
		await writeFile(join(testDir, "big.svg"), bigSvg);
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><img src="/big.svg"></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inline-svg": { cwd: testDir, maxBytes: 100 } },
		});
		const html = await readTestHtml("index.html");
		expect(html).toContain("<img");
	...
	it("does not inline external SVG URLs (https://)", async () => {
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><img src="https://example.com/icon.svg"></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inline-svg": { cwd: testDir } },
	...
	it("does not inline when data-no-inline is present", async () => {
		...
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><img src="/icon.svg" data-no-inline></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inline-svg": { cwd: testDir } },
	...
	it('adds aria-label and role="img" when alt is provided', async () => {
		...
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><img src="/icon.svg" alt="My Icon"></body></html>',
		);
		await prebuild({
			cwd: testDir,
			plugins: { "inline-svg": { cwd: testDir } },
	...
	it("strips width and height from the inlined SVG", async () => {
		await writeFile(
			join(testDir, "icon.svg"),
			'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"><circle r="10"/></svg>',
		);
		await writeTestHtml(
			"index.html",
			'<html><head></head><body><img src="/icon.svg"></body></html>',
