...
const NOJS_STYLE_ATTRS = ['data-nojs-animations', 'data-nojs-pending-css', 'data-nojs-skeleton-css', 'data-nojs-view-transitions'];
...
function purgeCSS(css, doc) {
  ...
  while (i < css.length) {
    ...
    if (wsMatch) { result.push(wsMatch[0]); i += wsMatch[0].length; continue; }
    ...
    if (!ruleMatch) { result.push(css.slice(i)); break; }
...
function extractBlock(css, start) {
  ...
  let foundOpen = false;
  while (i < css.length) {
    if (css[i] === '{') { depth++; foundOpen = true; }
    else if (css[i] === '}') { depth--; if (foundOpen && depth === 0) { i++; break; } }
    else if (!foundOpen && css[i] === ';') { i++; break; }  // single-line at-rule
    i++;
  }
