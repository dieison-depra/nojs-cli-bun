...
export default {
  ...
  async finalize({ outputDir, config }) {
    ...
    const base = typeof config.base === 'string'
      ? config.base.replace(/\/+$/, '') || '/'
      : '/'

    for (const target of targets) {
      switch (target) {
        case 'netlify': await writeNetlify(outputDir, base, useHashDetected); break
        case 'vercel':  await writeVercel(outputDir, base, useHashDetected); break
        case 'apache':  await writeApache(outputDir, base, useHashDetected); break
        case 'nginx':   await writeNginx(outputDir, base, useHashDetected); break
        default:
