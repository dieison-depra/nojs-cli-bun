...
const KEYFRAMES = new Map([
  ...
  ['fadeOutUp',     '@keyframes fadeOutUp { from { opacity: 1; transform: translateY(0); } to { opacity: 0; transform: translateY(-20px); } }'],
  ['fadeOutDown',   '@keyframes fadeOutDown { from { opacity: 1; transform: translateY(0); } to { opacity: 0; transform: translateY(20px); } }'],
  ['slideInLeft',   '@keyframes slideInLeft { from { transform: translateX(-100%); } to { transform: translateX(0); } }'],
  ['slideInRight',  '@keyframes slideInRight { from { transform: translateX(100%); } to { transform: translateX(0); } }'],
  ['slideOutLeft',  '@keyframes slideOutLeft { from { transform: translateX(0); } to { transform: translateX(-100%); } }'],
  ['slideOutRight', '@keyframes slideOutRight { from { transform: translateX(0); } to { transform: translateX(100%); } }'],
  ['zoomIn',        '@keyframes zoomIn { from { opacity: 0; transform: scale(0.5); } to { opacity: 1; transform: scale(1); } }'],
  ['zoomOut',       '@keyframes zoomOut { from { opacity: 1; transform: scale(1); } to { opacity: 0; transform: scale(0.5); } }'],
  ...
  ['bounceOut',     '@keyframes bounceOut { 0% { opacity: 1; transform: scale(1); } 20% { transform: scale(0.9); } 50%,55% { opacity: 1; transform: scale(1.1); } 100% { opacity: 0; transform: scale(0.3); } }'],
]);
