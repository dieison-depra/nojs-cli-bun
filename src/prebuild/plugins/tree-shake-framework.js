import { writeFile, unlink, readFile, access, rename } from 'node:fs/promises'
...
export default {
  ...
  async process(html) {
    ...
    for (const el of doc.querySelectorAll('*')) {
      for (const attr of el.getAttributeNames()) {
        ...
        if (attr.startsWith('on-'))   this._usedModules.add('directives/events.js')
        if (attr.startsWith('bind-')) this._usedModules.add('directives/binding.js')
      }
    }
    ...
    if (!this._scriptSrc) {
      for (const script of doc.querySelectorAll('script[src]')) {
        ...
        if (NOJS_SCRIPT_RE.test(src)) { this._scriptSrc = src; break }
      }
    }
  ...
  async finalize({ outputDir, config, processedFiles }) {
    ...
    const indexContent = await readFile(join(frameworkSrc, 'index.js'), 'utf-8')
    ...
    try {
      ...
      if (!result.success) {
        for (const log of result.logs) console.error('[tree-shake-framework]', log)
        return
      }
