...
export default {
	...
	async process(html, { _filePath, _config }) {
		...
		const routeAttrs = {};
		const defaultRoute = doc.querySelector(
			'template[route="/"], template[route="*"]',
		);
		if (defaultRoute)
			Object.assign(routeAttrs, collectDirectives(defaultRoute));
...
function collectDirectives(root) {
	...
	for (const [attr, key] of Object.entries(mapping)) {
		const el = root.matches?.(`[${attr}]`)
			? root
			: root.querySelector?.(`[${attr}]`);
		if (!el) continue;
		const literal = extractStringLiteral(el.getAttribute(attr));
		if (literal !== null) result[key] = literal;
	}

	const jsonldEl = root.matches?.("[page-jsonld]")
		? root
		: root.querySelector?.("[page-jsonld]");
	if (jsonldEl) {
