...
function buildIndex(answers) {
  ...
  const bodyAttrs = [];
  if (answers.apiUrl)
    bodyAttrs.push(`base="${escapeHtml(answers.apiUrl)}"`);
  if (answers.i18n)
    bodyAttrs.push(`store="app" state="{ locale: '${answers.defaultLocale || "en"}' }"`);
...
var CDN_URL = "https://cdn.no-js.dev/", LOCALE_RE;
...
import { dirname, join as join2, relative, resolve as resolve3 } from "path";
...
var init_decode_codepoint = __esm(() => {
  ...
  fromCodePoint = (_a = String.fromCodePoint) !== null && _a !== undefined ? _a : (codePoint) => {
    let output = "";
    if (codePoint > 65535) {
      codePoint -= 65536;
...
var init_escape = __esm(() => {
  ...
  getCodePoint = String.prototype.codePointAt != null ? (str, index) => str.codePointAt(index) : (c, index) => (c.charCodeAt(index) & 64512) === 55296 ? (c.charCodeAt(index) - 55296) * 1024 + c.charCodeAt(index + 1) - 56320 + 65536 : c.charCodeAt(index);
...
var init_constants = __esm(() => {
  BLOCK_ELEMENTS = new Set(["ARTICLE", "ASIDE", "BLOCKQUOTE", "BODY", "BR", "BUTTON", "CANVAS", "CAPTION", "COL", "COLGROUP", "DD", "DIV", "DL", "DT", "EMBED", "FIELDSET", "FIGCAPTION", "FIGURE", "FOOTER", "FORM", "H1", "H2", "H3", "H4", "H5", "H6", "LI", "UL", "OL", "P"]);
});
...
class CustomElementRegistry {
  ...
  define(localName, Class, options = {}) {
    ...
    if (waiting.has(localName)) {
      for (const resolve4 of waiting.get(localName))
        resolve4(Class);
  ...
  whenDefined(localName) {
    ...
    return new Promise((resolve4) => {
      if (registry.has(localName))
        resolve4(registry.get(localName).Class);
      else {
        if (!waiting.has(localName))
...
var reactive = false, Classes, customElements, attributeChangedCallback = (element, attributeName, oldValue, newValue) => {
...
var Parser2, notParsing = true, append2 = (self, node2, active) => {
  const end = self[END];
...
var htmlClasses, registerHTMLClass = (names, Class) => {
...
var emptyAttributes, setAttribute = (element, attribute2) => {
...
var { replace } = "", ca, esca, pe = (m) => esca[m], escape2 = (es) => replace.call(es, ca, pe);
...
var whitespace, ZERO = 48, NINE = 57;
...
var import_boolbase4, PLACEHOLDER_ELEMENT, is = (next, token, options, context, compileToken) => {
...
var import_boolbase6, defaultEquals = (a, b) => a === b, defaultOptions, compile3, _compileUnsafe, _compileToken, selectAll, selectOne;
var init_esm8 = __esm(() => {
  init_esm5();
  init_compile2();
  init_subselects();
...
var isArray, isTag3 = ({ nodeType }) => nodeType === ELEMENT_NODE, existsOne2 = (test, elements) => elements.some((element) => isTag3(element) && (test(element) || existsOne2(test, getChildren2(element)))), getAttributeValue2 = (element, name) => name === "class" ? element.classList.value : element.getAttribute(name), getChildren2 = ({ childNodes }) => childNodes, getName2 = (element) => {
  ...
  if (node2.nodeType === TEXT_NODE)
    return node2.data;
  return "";
}, hasAttrib2 = (element, name) => element.hasAttribute(name), removeSubsets2 = (nodes) => {
  let { length } = nodes;
  while (length--) {
  ...
  for (let node2 of nodes)
    if (test(node2) || (node2 = findOne2(test, getChildren2(node2))))
      return node2;
  return null;
}, adapter, prepareMatch = (element, selectors) => compile3(selectors, {
...
var isNode = (node2) => node2 instanceof Node2, insert = (parentNode, child, nodes) => {
...
var getInnerHtml = (node2) => node2.childNodes.join(""), setInnerHtml = (node2, html) => {
...
var refs, key = (name) => `data-${esm_default2(name)}`, prop = (name) => name.slice(5).replace(/-([a-z])/g, (_, $1) => $1.toUpperCase()), handler;
...
var refs2, getKeys = (style) => [...style.keys()].filter((key2) => key2 !== PRIVATE), updateKeys = (style) => {
  ...
  return attr;
}, handler2, CSSStyleDeclaration, prototype;
...
var BUBBLING_PHASE = 3, AT_TARGET = 2, CAPTURING_PHASE = 1, NONE = 0;
var init_event = () => {};
...
var init_lib = __esm(() => {
  ...
  $CSSMediaRule = require_CSSMediaRule().CSSMediaRule;
  $CSSSupportsRule = require_CSSSupportsRule().CSSSupportsRule;
  $CSSImportRule = require_CSSImportRule().CSSImportRule;
  $CSSFontFaceRule = require_CSSFontFaceRule().CSSFontFaceRule;
  $CSSHostRule = require_CSSHostRule().CSSHostRule;
  $StyleSheet = require_StyleSheet().StyleSheet;
  $CSSStyleSheet = require_CSSStyleSheet().CSSStyleSheet;
  $CSSKeyframesRule = require_CSSKeyframesRule().CSSKeyframesRule;
  $CSSKeyframeRule = require_CSSKeyframeRule().CSSKeyframeRule;
  $MatcherList = require_MatcherList().MatcherList;
  $CSSDocumentRule = require_CSSDocumentRule().CSSDocumentRule;
  $CSSValue = require_CSSValue().CSSValue;
  $CSSValueExpression = require_CSSValueExpression().CSSValueExpression;
...
}, globalExports, window, Document2;
...
var init_audit_accessibility = __esm(() => {
  ...
  audit_accessibility_default = {
    ...
    description: "WCAG quick-check: alt text, link names, labels, heading order, lang.",
    async process(html, { filePath, config }) {
      ...
      if (rules["link-name"] !== false) {
        for (const a of doc.querySelectorAll("a[href]")) {
          ...
          if (!text && !a.getAttribute("aria-label") && !a.getAttribute("aria-labelledby")) {
            violations.push(`[link-name] <a> has no accessible name in ${label}`);
          }
        }
      }
      if (rules.label !== false) {
        const labelledIds = new Set([...doc.querySelectorAll("label[for]")].map((l) => l.getAttribute("for")));
        for (const input of doc.querySelectorAll('input:not([type="hidden"]):not([type="button"]):not([type="submit"]):not([type="reset"])')) {
          const id = input.getAttribute("id");
          const hasLabel = id && labelledIds.has(id) || !!input.closest("label");
          const hasAria = input.getAttribute("aria-label") || input.getAttribute("aria-labelledby");
          if (!hasLabel && !hasAria) {
            violations.push(`[label] <input> has no associated label in ${label}`);
          }
        }
      }
      if (rules["heading-order"] !== false) {
        const headings = [...doc.querySelectorAll("h1,h2,h3,h4,h5,h6")].map((h) => parseInt(h.tagName.slice(1), 10));
        let prev = 0;
        for (const level of headings) {
          if (level > prev + 1 && prev !== 0) {
            violations.push(`[heading-order] heading level skipped (h${prev}\u2192h${level}) in ${label}`);
          }
          prev = level;
        }
      }
      if (rules["html-lang"] !== false) {
        ...
        if (htmlEl && !htmlEl.getAttribute("lang")) {
          violations.push(`[html-lang] <html> missing lang attribute in ${label}`);
        }
      }
      if (rules["landmark-main"] !== false) {
        const hasMain = doc.querySelector("main") || doc.querySelector('[role="main"]');
        if (!hasMain) {
          violations.push(`[landmark-main] document has no <main> landmark in ${label}`);
        }
      }
      if (rules.list !== false) {
        const ALLOWED = new Set(["li", "script", "template"]);
        for (const list of doc.querySelectorAll("ul, ol, menu")) {
          if (list.closest("template"))
            continue;
          for (const child of list.children) {
            if (!ALLOWED.has(child.tagName.toLowerCase())) {
              violations.push(`[list] <${list.tagName.toLowerCase()}> contains <${child.tagName.toLowerCase()}> (not a list item) in ${label}`);
              break;
            }
          }
        }
      ...
      if (rules.listitem !== false) {
        ...
        for (const li of doc.querySelectorAll("li")) {
          if (li.closest("template"))
            continue;
          const parent = li.parentElement;
          if (!parent || !LIST_PARENTS.has(parent.tagName.toLowerCase())) {
            violations.push(`[listitem] <li> is not inside <ul>, <ol> or <menu> in ${label}`);
          }
        }
      }
...
`);
        ...
        if (failOnError) {
          throw new Error(`audit-accessibility: ${violations.length} violation(s) found`);
        }
      }
      return html;
    }
...
var init_audit_meta_tags = __esm(() => {
  ...
  audit_meta_tags_default = {
    ...
    description: "Check and inject required meta tags (charset, viewport, title, description, lang).",
    async process(html, { filePath, config }) {
      const opts = config || {};
      const shouldInject = opts.inject !== false;
      const failOnError = opts.failOnError === true;
      const isPartial = !html.toLowerCase().includes("<html") && !html.toLowerCase().includes("<body");
      const { document: doc } = parseHTML(html);
      const head = doc.head;
      if (!head)
        return html;
      ...
      if (!head.querySelector('meta[name="description"]')) {
        if (shouldInject && !isPartial) {
          ...
          const titleText = head.querySelector("title")?.textContent || "No.JS Application";
          meta.setAttribute("content", `${titleText} - built with No.JS framework.`);
          head.appendChild(meta);
          changed = true;
        } else if (!isPartial) {
          issues.push(`missing <meta name="description"> in ${filePath || "unknown"}`);
        }
      }
      const htmlEl = doc.documentElement || doc.querySelector("html");
      if (htmlEl && !htmlEl.getAttribute("lang")) {
        if (shouldInject && !isPartial) {
          ...
          issues.push(`missing lang attribute on <html> in ${filePath || "unknown"}`);
        }
      }
...
// src/prebuild/plugins/compile-templates-to-js.js
import { writeFile as writeFile3 } from "fs/promises";
import { join as join3 } from "path";
var compile_templates_to_js_default;
var init_compile_templates_to_js = __esm(() => {
  init_esm9();
  compile_templates_to_js_default = {
...
`.trim();
    ...
    async finalize({ outputDir }) {
      ...
      for (const [route, data] of this._templates) {
        ...
        await writeFile3(filePath, data.js, "utf-8");
        routesManifest[route] = `./${fileName}`;
      }
      const manifestPath = join3(outputDir, "routes-manifest.js");
      const manifestContent = `export default ${JSON.stringify(routesManifest, null, 2)};`;
      await writeFile3(manifestPath, manifestContent, "utf-8");
      console.log(`[compile-templates-to-js] Compiled ${this._templates.size} templates and generated manifest.`);
    ...
    async finalize({ outputDir, processedFiles }) {
      for (const htmlPath of processedFiles) {
        ...
        for (const script of scripts) {
          ...
          const inputPath = join4(outputDir, src);
          try {
            await Bun.build({
        ...
        if (changed) {
          await writeFile4(htmlPath, doc.toString(), "utf-8");
        }
      }
...
import { readdir, readFile as readFile3, rename, writeFile as writeFile5 } from "fs/promises";
import { basename as basename2, dirname as dirname2, extname as extname2, join as join5 } from "path";
  ...
  fingerprint_assets_default = {
    ...
    async finalize({ outputDir, processedFiles }) {
      ...
      for (const htmlPath of processedFiles) {
        ...
        if (changed) {
          await writeFile5(htmlPath, html, "utf-8");
        }
      }
      const headersPath = join5(outputDir, "_headers");
      try {
        ...
        if (changed) {
          await writeFile5(headersPath, headers, "utf-8");
        }
      } catch {}
...
import { readdir as readdir2, stat, writeFile as writeFile6 } from "fs/promises";
import { join as join6, relative as relative2 } from "path";
async function getFilesRecursively2(dir) {
  const entries2 = await readdir2(dir, { withFileTypes: true });
  const files = await Promise.all(entries2.map((res) => {
    const resPath = join6(dir, res.name);
...
var init_generate_bundle_report = __esm(() => {
  generate_bundle_report_default = {
    ...
    async finalize({ outputDir }) {
      ...
      const reportPath = join6(outputDir, "nojs-bundle-report.html");
      await writeFile6(reportPath, reportHtml, "utf-8");
...
`;
  const dest = join7(outputDir, "_redirects");
...
`;
  const dest = join7(outputDir, "vercel.json");
  await writeFile7(dest, content, "utf-8");
  const note = useHashDetected ? " (note: useHash mode detected \u2014 may not be needed)" : "";
...
`);
  const dest = join7(outputDir, ".htaccess");
  await writeFile7(dest, content, "utf-8");
  const note = useHashDetected ? " (note: useHash mode detected \u2014 may not be needed)" : "";
...
`);
  const dest = join7(outputDir, "nginx.conf");
  await writeFile7(dest, content, "utf-8");
  const note = useHashDetected ? " (note: useHash mode detected \u2014 may not be needed)" : "";
...
var init_generate_deploy_config = __esm(() => {
  ...
  generate_deploy_config_default = {
    ...
    description: "Generate SPA fallback deployment configs for Netlify, Vercel, Apache, or nginx",
    async process(html, { filePath }) {
      if (this._useHashDetected)
        return html;
      const { document: doc } = parseHTML(html);
      for (const script of doc.querySelectorAll("script:not([src])")) {
        if (USEHASH_RE.test(script.textContent || "")) {
          this._useHashDetected = true;
          console.warn(`[generate-deploy-config] warn: useHash: true detected in ${filePath} \u2014 ` + "SPA fallback configs are ineffective in hash mode");
          break;
        }
      }
      return html;
    ...
    async finalize({ outputDir, config }) {
      ...
      const targets = Array.isArray(config.targets) ? config.targets : ["netlify"];
      const base = typeof config.base === "string" ? config.base.replace(/\/+$/, "") || "/" : "/";
      for (const target of targets) {
        switch (target) {
          ...
          case "nginx":
            ...
            break;
          default:
            console.warn(`[generate-deploy-config] unknown target "${target}" \u2014 skipped`);
        }
      }
    }
  };
...
function getPublicPath(outputPath, outputDir) {
  const rel = `/${relative3(outputDir, outputPath).replace(/\\/g, "/")}`;
  if (rel === "/index.html")
    return "/";
...
`;
      ...
      if (content) {
        const dest = join8(outputDir, "_headers");
        await writeFile8(dest, content, "utf-8");
...
var init_generate_pwa_manifest = __esm(() => {
  ...
  generate_pwa_manifest_default = {
    ...
    async finalize({ outputDir, config }) {
      ...
      const dest = join9(outputDir, "manifest.webmanifest");
      await writeFile9(dest, JSON.stringify(manifest, null, 2), "utf-8");
    }
  };
});

// src/prebuild/plugins/generate-responsive-images.js
import { basename as basename3, dirname as dirname3, extname as extname3 } from "path";
...
var init_generate_responsive_images = __esm(() => {
  ...
  generate_responsive_images_default = {
    ...
    description: "Generate AVIF/WebP responsive image variants and update srcset.",
    async process(html, { config }) {
      let sharp;
      try {
...
import { readdir as readdir3, writeFile as writeFile10 } from "fs/promises";
...
`.trim();
    ...
    async finalize({ outputDir, config }) {
      ...
      const assetsToCache = allFiles.map((f) => `/${relative4(outputDir, f).replace(/\\/g, "/")}`).filter((f) => {
        const skip = [
          swName,
          "_headers",
          "_redirects",
...
var init_generate_sitemap = __esm(() => {
  ...
  generate_sitemap_default = {
    ...
    async finalize({ outputDir, config }) {
      ...
      const urls = sortedRoutes.filter((r) => !excludePatterns.some((p) => r.includes(p))).map((route) => {
  ...
  hoist_static_content_default = {
    ...
    async process(html) {
      ...
      function checkDynamic(node2) {
        if (isNodeDynamic.has(node2))
          return isNodeDynamic.get(node2);
        if (node2.nodeType === 3) {
        ...
        if (node2.tagName === "TEMPLATE" && node2.content) {
          children.push(...Array.from(node2.content.childNodes));
        }
...
// src/prebuild/plugins/identify-islands.js
var INTERACTIVE_ATTRS, identify_islands_default;
var init_identify_islands = __esm(() => {
  init_esm9();
  INTERACTIVE_ATTRS = [
    "state",
    "store",
  ...
  identify_islands_default = {
    ...
    async process(html) {
      ...
      for (const el of allElements) {
        const hasDirective = el.getAttributeNames().some((attr) => INTERACTIVE_ATTRS.some((prefix) => attr.startsWith(prefix)));
        if (hasDirective) {
...
var init_inject_canonical_url = __esm(() => {
  ...
  inject_canonical_url_default = {
    ...
    async process(html, { filePath, config }) {
      ...
      const rel = filePath ? relative5(cwd, filePath) : "index.html";
...
var init_inject_csp_hashes = __esm(() => {
  ...
  inject_csp_hashes_default = {
    ...
    description: "Compute SHA-384 hashes for inline scripts/styles and inject a CSP meta tag.",
    async process(html, { config }) {
      const opts = config || {};
      ...
      for (const el of doc.querySelectorAll("script:not([src])")) {
        if (el.getAttribute("type") === "speculationrules")
          continue;
      ...
      let cspMeta = head.querySelector('meta[http-equiv="Content-Security-Policy"]');
      if (cspMeta) {
...
var init_inject_event_delegation = __esm(() => {
  ...
  inject_event_delegation_default = {
    ...
    description: "Transform individual event listeners into root-delegated events for better performance",
    async process(html, { config = {} } = {}) {
      const { document: doc } = parseHTML(html);
      const head = doc.head;
      if (!head)
...
import { dirname as dirname4, join as join12, relative as relative6 } from "path";
...
async function findWebRoot(startDir, webPath) {
  ...
  for (;; ) {
    try {
      await readFile4(join12(dir, rel));
      return dir;
    } catch {
...
async function collectImports(filePath, webRoot, visited) {
  let src;
  try {
    src = await readFile4(filePath, "utf8");
  } catch {
    return [];
  }
  const results = [];
  ...
  while ((m = IMPORT_RE.exec(src)) !== null) {
    ...
    if (!specifier.startsWith("."))
      continue;
    const absPath = join12(fileDir, specifier);
    if (visited.has(absPath))
      continue;
    visited.add(absPath);
    results.push("/" + relative6(webRoot, absPath));
    const nested = await collectImports(absPath, webRoot, visited);
    results.push(...nested);
  }
  return results;
}
var inject_modulepreload_default, IMPORT_RE;
var init_inject_modulepreload = __esm(() => {
  init_esm9();
  inject_modulepreload_default = {
    name: "inject-modulepreload",
    description: "Inject modulepreload hints for module scripts and their transitive imports to reduce load latency.",
    async process(html, { filePath } = {}) {
      const { document: doc } = parseHTML(html);
      if (!doc.head)
        return html;
      const existing = new Set([...doc.querySelectorAll('link[rel="modulepreload"]')].map((el) => el.getAttribute("href")));
      const scripts = doc.querySelectorAll('script[type="module"][src]');
      let changed = false;
      for (const script of scripts) {
        const src = script.getAttribute("src");
        if (!src || isInterpolated(src) || isCrossOrigin(src))
          continue;
        if (!existing.has(src)) {
        ...
        if (!filePath)
          continue;
        const webRoot = await findWebRoot(dirname4(filePath), src);
        if (!webRoot)
          continue;
        const entryPath = join12(webRoot, src.startsWith("/") ? src.slice(1) : src);
        const imports = await collectImports(entryPath, webRoot, new Set([entryPath]));
        for (const webPath of imports) {
          if (existing.has(webPath))
            continue;
...
function setMeta(doc, head, attrType, name, content) {
  if (head.querySelector(`meta[${attrType}="${name}"]`))
    return;
  const meta = doc.createElement("meta");
...
var init_inject_og_twitter = __esm(() => {
  ...
  inject_og_twitter_default = {
    ...
    async process(html, { _filePath, config }) {
      ...
      if (!head)
        return html;
      const title = getStaticPageAttr(doc, "page-title") || head.querySelector("title")?.textContent?.trim();
      const description = getStaticPageAttr(doc, "page-description") || head.querySelector('meta[name="description"]')?.getAttribute("content");
      const canonical = getStaticPageAttr(doc, "page-canonical") || head.querySelector('link[rel="canonical"]')?.getAttribute("href");
      setMeta(doc, head, "property", "og:type", config.type || "website");
      if (title)
        setMeta(doc, head, "property", "og:title", title);
      if (description)
        setMeta(doc, head, "property", "og:description", description);
      if (canonical)
        setMeta(doc, head, "property", "og:url", canonical);
      ...
      if (title)
        setMeta(doc, head, "name", "twitter:title", title);
      if (description)
  ...
  inject_resource_hints_default = {
    ...
    async process(html, { _filePath, config }) {
      ...
      for (const el of fetchEls) {
        ...
        if (!url || isInterpolated2(url) || existingHrefs.has(url))
          continue;
      ...
      for (const tpl of routeTemplates) {
        ...
        if (!src || isInterpolated2(src) || existingHrefs.has(src))
          continue;
      ...
      if (config.apiBase) {
        try {
          ...
          const existingDnsPrefetch = new Set([...head.querySelectorAll('link[rel="dns-prefetch"]')].map((el) => el.getAttribute("href")));
...
var init_inject_speculation_rules = __esm(() => {
  ...
  inject_speculation_rules_default = {
    ...
    description: "Inject Speculation Rules API based on No.JS route definitions for near-instant navigation",
    async process(html, { _filePath, config }) {
      const { document: doc } = parseHTML(html);
      const head = doc.head;
      if (!head)
        return html;
      if (head.querySelector('script[type="speculationrules"]'))
        return html;
      const routeTemplates = doc.querySelectorAll("template[route]");
      const routes = [];
      const excludePatterns = config.excludePatterns || [];
      for (const tpl of routeTemplates) {
        ...
        if (!route || route === "*" || route.includes(":"))
          continue;
        if (excludePatterns.some((p) => route.includes(p)))
          continue;
        routes.push(route);
      }
      if (routes.length === 0)
...
var init_inject_sri_hashes = __esm(() => {
  ...
  inject_sri_hashes_default = {
    ...
    description: "Add integrity (SHA-384) and crossorigin to external scripts and stylesheets.",
    async process(html, { config }) {
      const opts = config || {};
...
var SLOT_ATTRS, SKELETON_CSS_ATTR = "data-nojs-skeleton-css", SKELETON_CSS, inject_template_hints_default;
var init_inject_template_hints = __esm(() => {
  init_esm9();
  SLOT_ATTRS = ["loading", "skeleton", "error", "empty", "success"];
  SKELETON_CSS = ".skeleton{background:#e0e0e0;border-radius:4px;animation:skeleton-pulse 1.5s ease-in-out infinite}" + "@keyframes skeleton-pulse{0%,100%{opacity:1}50%{opacity:.4}}";
  inject_template_hints_default = {
    name: "inject-template-hints",
    description: "Preload stylesheets referenced by loading/skeleton/error templates and inline skeleton animation CSS",
    async process(html) {
      const { document: doc } = parseHTML(html);
      const head = doc.head;
      if (!head)
      ...
      for (const id of templateIds) {
        ...
        for (const el of root?.querySelectorAll("[class]") ?? []) {
          for (const cls of el.classList) {
            if (/skeleton/i.test(cls))
              hasSkeletonClass = true;
          }
        }
      }
      let modified = false;
      const existingPreloads = new Set([...head.querySelectorAll('link[rel="preload"][as="style"]')].map((l) => l.getAttribute("href")));
...
var init_inject_view_transitions = __esm(() => {
  ...
  inject_view_transitions_default = {
    ...
    async process(html, { config }) {
      ...
      if (!doc.head)
        return html;
      if (doc.head.querySelector("[data-nojs-view-transitions]"))
        return html;
      const style = doc.createElement("style");
...
var PENDING_ATTR = "data-nojs-pending", STYLE_ATTR = "data-nojs-pending-css", PENDING_CSS, inject_visibility_css_default;
var init_inject_visibility_css = __esm(() => {
  ...
  inject_visibility_css_default = {
    ...
    async process(html) {
      ...
      if (!head)
        return html;
      const candidates = [...doc.querySelectorAll("[if], [show], [hide]")].filter((el) => !isInsideTemplate(el));
...
var init_inline_animation_css = __esm(() => {
  ...
  inline_animation_css_default = {
    ...
    description: "Inline only the animation keyframes used in the document to prevent FOUC on first paint",
    async process(html) {
      const { document: doc } = parseHTML(html);
      const head = doc.head;
      if (!head)
        return html;
      if (head.querySelector("[data-nojs-animations]"))
        return html;
      const names = new Set;
...
async function findWebRoot2(startDir, webPath) {
  if (!webPath.startsWith("/"))
    return startDir;
  const rel = webPath.slice(1);
  let dir = startDir;
  for (;; ) {
    try {
      await readFile5(join13(dir, rel));
      return dir;
    } catch {
...
var init_inline_css = __esm(() => {
  ...
  inline_css_default = {
    name: "inline-css",
    description: "Inline local CSS stylesheets into <style> tags to eliminate render-blocking requests.",
    async process(html, { filePath, config } = {}) {
      const opts = config || {};
      ...
      for (const link of links) {
        const href = link.getAttribute("href");
        if (!href || isCrossOrigin3(href))
          continue;
        const webRoot = filePath ? await findWebRoot2(dirname5(filePath), href) : null;
        if (!webRoot)
          continue;
        const cssPath = join13(webRoot, href.startsWith("/") ? href.slice(1) : href);
        let css;
        try {
          css = await readFile5(cssPath, "utf8");
        } catch {
          continue;
        }
...
var init_inline_svg = __esm(() => {
  inline_svg_default = {
    ...
    async process(html, { filePath, config }) {
      ...
      const cwd = opts.cwd || (filePath ? filePath.replace(/\/[^/]+$/, "") : process.cwd());
      ...
      for (const match of html.matchAll(IMG_RE)) {
        ...
        if (/data-no-inline/.test(attrs))
          continue;
        const absPath = src.startsWith("/") ? join14(cwd, src) : resolve4(cwd, src);
        let fileSize;
        try {
          fileSize = (await stat2(absPath)).size;
        } catch {
          continue;
        }
        ...
        let svgContent;
        try {
          svgContent = await readFile6(absPath, "utf-8");
        } catch {
          continue;
        }
        svgContent = svgContent.replace(/<\?xml[^?]*\?>/i, "").replace(/<!DOCTYPE[^>]*>/i, "").trim();
        const altMatch = attrs.match(/alt="([^"]*)"/i);
        const alt = altMatch ? altMatch[1] : null;
        if (alt) {
          svgContent = svgContent.replace("<svg", `<svg aria-label="${alt.replace(/"/g, "&quot;")}" role="img"`);
        } else {
...
var init_minify_html = __esm(() => {
  minify_html_default = {
    ...
    async process(html, { config }) {
      ...
      protected_html = protected_html.replace(/\uE000PRESERVED_(\d+)\uE000/g, (_, i) => preserved[Number(i)]);
...
var GFONTS_API = "https://fonts.googleapis.com", GFONTS_STATIC = "https://fonts.gstatic.com", optimize_fonts_default;
var init_optimize_fonts = __esm(() => {
  ...
  optimize_fonts_default = {
    ...
    async process(html) {
      ...
      for (const style of head.querySelectorAll("style")) {
        ...
        if (text.includes("fonts.googleapis.com")) {
          ...
          if (!text.includes("display=")) {
            style.textContent = text.replace(/(fonts\.googleapis\.com\/css[^'")]*)/g, (match) => match.includes("?") ? `${match}&display=swap` : `${match}?display=swap`);
            changed = true;
          }
        }
      }
      if (!hasGoogleFonts)
        return html;
      const existingPreconnects = new Set([...head.querySelectorAll('link[rel="preconnect"]')].map((el) => el.getAttribute("href")));
...
function detectLcpCandidate(images, lcpSelector, doc) {
  if (lcpSelector)
    return doc.querySelector(lcpSelector);
  for (const img of images) {
    if (LCP_CLASS_RE.test(img.getAttribute("class") || ""))
      return img;
  }
...
var init_optimize_images = __esm(() => {
  ...
  optimize_images_default = {
    ...
    description: "Add lazy loading, LCP preload, and fetchpriority hints to images",
    async process(html, { filePath, config }) {
      const { document: doc } = parseHTML(html);
      const head = doc.head;
      if (!head)
      ...
      const lcpImage = detectLcpCandidate(images, config.lcpSelector || null, doc);
      for (const img of images) {
        ...
        if (img === lcpImage) {
          ...
          if (skipLazy && img.matches(skipLazy))
            continue;
...
import { readdir as readdir4, readFile as readFile7, writeFile as writeFile12 } from "fs/promises";
import { extname as extname4, join as join15 } from "path";
...
var brotliCompressAsync, gzipAsync, DEFAULT_EXTENSIONS, precompress_assets_default;
  ...
  precompress_assets_default = {
    ...
    async finalize({ outputDir, config }) {
      ...
      await Promise.all(files.map(async (filePath) => {
        ...
        const tasks = [];
        if (doBrotli) {
          tasks.push(brotliCompressAsync(content).then((buf) => writeFile12(`${filePath}.br`, buf)));
        }
        if (doGzip) {
          tasks.push(gzipAsync(content).then((buf) => writeFile12(`${filePath}.gz`, buf)));
        }
  ...
  purge_unused_css_default = {
    ...
    async process(html) {
      ...
      for (const style of doc.querySelectorAll("style")) {
        if (NOJS_STYLE_ATTRS.some((attr) => style.hasAttribute(attr)))
          continue;
...
import { access, readFile as readFile8, rename as rename2, unlink, writeFile as writeFile13 } from "fs/promises";
...
function stripUnusedImports(src, keep, frameworkSrc) {
  return src.replace(/^import "\.\/([^"]+)";\n/gm, (_match, modPath) => {
    if (SIDE_EFFECT_MODULES.has(modPath) && !keep.has(modPath))
      return "";
    return `import "${join16(frameworkSrc, modPath)}";
`;
  }).replace(/^(import\s+.*?from\s+)"\.\/([^"]+)"/gm, (_match, prefix, modPath) => {
    return `${prefix}"${join16(frameworkSrc, modPath)}"`;
  });
}
async function findFrameworkSrc(outputDir, config) {
  if (config.frameworkSrc) {
    const p = resolve5(config.frameworkSrc);
    if (await pathExists(join16(p, "index.js")))
      return p;
  }
  let dir = outputDir;
  for (let i = 0;i < 8; i++) {
    const candidate = join16(dir, "node_modules", "@erickxavier", "no-js", "src");
    if (await pathExists(join16(candidate, "index.js")))
      return candidate;
...
var ATTR_MODULE_MAP, SIDE_EFFECT_MODULES, NOJS_SCRIPT_RE, tree_shake_framework_default;
var init_tree_shake_framework = __esm(() => {
  ...
  tree_shake_framework_default = {
    ...
    description: "Build a minimal no-js bundle with only the directives used in your HTML",
    ...
    async finalize({ outputDir, config, processedFiles }) {
      if (!this._scriptSrc) {
        console.warn("[tree-shake-framework] no no-js script tag found \u2014 skipping");
        return this._reset();
      }
      const frameworkSrc = await findFrameworkSrc(outputDir, config);
      if (!frameworkSrc) {
        console.warn(`[tree-shake-framework] @erickxavier/no-js source not found \u2014 skipping.
` + "  Install the package or set config.frameworkSrc to the src/ directory.");
      ...
      const indexContent = await readFile8(join16(frameworkSrc, "index.js"), "utf-8");
      const entry = stripUnusedImports(indexContent, keep, frameworkSrc);
      const entryPath = join16(outputDir, ".nojs-tree-entry.tmp.js");
      const bundleName = "nojs.bundle.js";
      await writeFile13(entryPath, entry, "utf-8");
      try {
        ...
        if (builtPath && builtPath !== join16(outputDir, bundleName)) {
          await rename2(builtPath, join16(outputDir, bundleName));
        }
        const srcEscaped = this._scriptSrc.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const scriptRe = new RegExp(`(<script[^>]*\\ssrc=")${srcEscaped}"`, "g");
        for (const filePath of processedFiles) {
          const html = await readFile8(filePath, "utf-8");
        ...
        console.log(`[tree-shake-framework] ${join16(outputDir, bundleName)}: built` + `
...
var init_plugins = __esm(() => {
  ...
  builtinPlugins = {
    ...
    [generate_responsive_images_default.name]: generate_responsive_images_default,
    ...
    [compile_templates_to_js_default.name]: compile_templates_to_js_default,
    [differential_serving_default.name]: differential_serving_default,
    [identify_islands_default.name]: identify_islands_default,
...
function resolvePlugins(pluginConfig) {
  ...
  for (const [name, value] of Object.entries(pluginConfig)) {
    ...
    if (!plugin) {
      throw new Error(`Unknown plugin: "${name}". Available: ${Object.keys(builtinPlugins).join(", ")}`);
    }
    active.push(plugin);
  }
  return active;
...
`);
  ...
  console.log(`Done. Processed ${result.files} file(s) with ${result.plugins.length} plugin(s): ${result.plugins.join(", ") || "(none)"}`);
}
  ...
  for (let i = 0;i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case "--dry-run":
        ...
        break;
      default:
        throw new Error(`Unknown option: ${arg}. Run "nojs prebuild --help" for usage.`);
    }
  }
  return args;
}
...
import { extname as extname5, join as join17, resolve as resolve6 } from "path";
  ...
  if (liveReload) {
    ...
    const watcher = watch(root, { recursive: true }, (_eventType, filename) => {
      ...
      debounceTimer = setTimeout(() => {
        if (!quiet)
          console.log(`  ${DIM}reload${RESET} ${filename} changed`);
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      try {
        ...
        if (!quiet)
          logRequest(req.method, url.pathname, 400, "bad encoding");
      ...
      if (pathname.includes("\x00")) {
        if (!quiet)
          logRequest(method, pathname, 400, "null byte");
        return new Response("Bad request", {
          status: 400,
          headers: { "Content-Type": "text/plain" }
        });
...
`);
      ...
      try {
        let filePath = join17(root, pathname);
        ...
        if (fileExists) {
          ...
          if (fileStat.isDirectory()) {
            filePath = join17(filePath, "index.html");
            fileExists = await exists(filePath);
          }
        }
        if (!fileExists && !extname5(pathname)) {
          filePath = join17(root, "index.html");
          fileExists = await exists(filePath);
        }
        if (!fileExists) {
          if (!quiet)
        ...
        if (!quiet)
          logRequest(req.method, pathname, 500, err.message);
...
var MIME_TYPES, MAX_SSE_CLIENTS = 100, LIVE_RELOAD_SCRIPT = `
<script>
(function() {
  function connect() {
...
`, STATUS_COLORS, RESET = "\x1B[0m", DIM = "\x1B[2m";
    ...
    switch (arg) {
      ...
      default:
        if (!arg.startsWith("-"))
          ...
          throw new Error(`Unknown option: ${arg}. Run "nojs dev --help" for usage.`);
    }
  }
  return args;
}
...
var init_rules = __esm(() => {
  ...
  RULES = [
    ...
    },
    {
      ...
      severity: "error",
      test(el) {
        if (!el.hasAttribute("validate") || el.tagName === "FORM")
          return null;
    ...
    },
    {
      ...
      test(doc) {
        ...
        for (const el of stores) {
          ...
          if (names.has(name)) {
            issues.push(`Duplicate store name "${name}". The second declaration may be ignored if the store was initialized via NoJS.config().`);
          }
          names.set(name, el);
        }
...
function parseArgs3(argv) {
  ...
  for (let i = 0;i < argv.length; i++) {
    ...
    if (arg === "--format")
      ...
      throw new Error(`Unknown option: ${arg}. Run "nojs validate --help" for usage.`);
  }
  return args;
}
var HELP4 = `
...
async function searchCdn(query2) {
  try {
    ...
    return (registry.plugins || []).filter((p) => p.name.toLowerCase().includes(q) || p.description?.toLowerCase().includes(q)).map((p) => ({ ...p, source: "cdn" }));
  } catch {
    return [];
  }
}
...
import { readFile as readFile9, writeFile as writeFile14 } from "fs/promises";
...
async function saveProjectConfig(config) {
  const configPath = resolve7(process.cwd(), CONFIG_FILE);
  await writeFile14(configPath, `${JSON.stringify(config, null, 2)}
`, "utf-8");
}
...
async function run5(argv) {
  ...
  switch (action) {
    case "search":
      if (!args[0])
        throw new Error("Usage: nojs plugin search <query>");
      await search(args[0]);
      break;
    case "install":
      if (!args[0])
        throw new Error("Usage: nojs plugin install <name>");
      await install(args[0]);
      break;
    case "update":
      if (!args[0])
        throw new Error("Usage: nojs plugin update <name>");
      await update2(args[0]);
      break;
    case "remove":
      if (!args[0])
        throw new Error("Usage: nojs plugin remove <name>");
...
var COMMANDS = {
  ...
  prebuild: () => Promise.resolve().then(() => (init_prebuild(), exports_prebuild)),
  dev: () => Promise.resolve().then(() => (init_dev(), exports_dev)),
  validate: () => Promise.resolve().then(() => (init_validate(), exports_validate)),
