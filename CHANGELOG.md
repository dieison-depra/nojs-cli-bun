...
- **Template Factory Preservation** — Fixed `hoist-static-content` plugin to correctly skip `<template>` tags and their internal content. This prevents the framework from incorrectly marking dynamic factories as static, which was causing visibility and binding issues in the runtime.
- **Robust Event Delegation** — Improved the injected delegation script to traverse the DOM tree more reliably using parent node lookups instead of just `closest()`, ensuring nested elements (like icons inside buttons) trigger events correctly.
